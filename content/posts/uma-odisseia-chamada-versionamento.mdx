...
Porém, eu não contava que esse processo seria mais pesado do que eu imaginava (mais complexo que no ambiente web).
...
- <strong>Entrave</strong>: A Play Store não aceitava o API Level abaixo de 33 (ou Android abaixo de 13)
- <strong>Solução</strong>: Atualizar o app feito em React Native da versão 0.68.2 para 0.72.4
...
Depois de pesquisar como adequar meu app a esse novo Android, cheguei a algumas soluções mas não consegui fugir de ter que atualizar o próprio `react-native`.
Talvez houvesse outras formas de resolver, mas a decisão foi tomada, então vamos seguir.

Rodei um `react-native-upgrade` e como sabia que teria que atualizar uma série de arquivos nativos, utilizei o [Upgrade helper](https://react-native-community.github.io/upgrade-helper/?from=0.68.2&to=0.72.4) como guia. 
Essa ferramenta foi uma mão na roda, obrigado comunidade 🙏<br/>

Depois de implementar as muitas correções arquivo por aquivo, o primeiro erro que surgiu foi esse: <br/>
```html
...
Após corrigir me guiando pelo próprio Upgrade Helper e com a atualização manual do [expo-modules](https://docs.expo.dev/bare/installing-expo-modules/)
...
Depois dessa correção(que não lembro como foi), surge outro:
>Build file '.../updated-app/node_modules/expo-application/android/build.gradle' line: 40

Como esse foi mais específico, a correção foi mais fácil, me levando a esse aviso: `classifier undefined`
...
Como o último fix não rolou, então usei o `npx expo-doctor` (lista exatamente as libs que precisam ser atualizadas)<br/>
...
Junto a esse erro, ao rodar o expo-doctor ele apontava que eu estava utilizando uma versão x do `@expo/config-plugins` e queria a versão y, sendo que eu tava com a versao y<br/>
...
Atualizei a versão das dependências de cada uma das dependências no `yarn.lock` e o `expo-doctor` não apontou mais nada, porém, segue o mesmo erro ao rodar `yarn android`
...
Observação: muito provavelmente são esses caras que estavam ferrando o rolê<br/>
- react-native-iap: "^12.10.7"
- @react-native-firebase/app: "^16.4.6"

Após avaliar o código, percebi que a lib `react-native-iap` não tava sendo usada, então comentei todas as menções a essa lib na codebase e a build funcionou.<br/>
...
Já é o início do quarto dia debugando isso, o foco já não é mais o mesmo.<br/>
...
tive que adicionar mais um bloco no `andrid/app/build.gradle` pois tava gerando os bundles com as configurações de debug, e como meu app é assinado diretamente pela loja e não uma key física, criei mais um bloco de configurações para bundles de produção.

e funcionou. 🙌

## Hora de submeter o app pra Loja

Sim, teve mais erros. A loja ainda apontava para um erro: "Não é possível lançar esta versão porque ela não permite que os usuários existentes façam a atualização para os novos pacotes de apps."

Pra resolver isso, bastou eu ver qual a última versão do app eu tinha submetido para a loja e corrigir colocando uma versão maior no `build.gradle`.

Agora sim, resolvido!

## Conclusão

Essa brincadeira levou 4 dias e meio, portanto, nunca subestimem uma atualização. Espero ter conseguido mostrar o lado não tão glamuroso da programação e também como foi surgindo a resolução de cada problema.
